...
internal static class Avx2ComplexKernels
{
    ...
    public static void CopyScale(ReadOnlySpan<ComplexF> source, float scale, Span<ComplexF> destination)
    {
        ...
        {
            var values = Vector256.LoadUnsafe(ref sourceReference, (nuint)index);
            Avx.Multiply(values, vectorScale).StoreUnsafe(ref destinationReference, (nuint)index);
        }
        ...
        Span<ComplexF> destination)
    {
        ...
        {
            var leftValues = Vector256.LoadUnsafe(ref leftReference, (nuint)index);
            var rightValues = Vector256.LoadUnsafe(ref rightReference, (nuint)index);
        ...
        Span<ComplexF> destination)
    {
             ...
             complexIndex += ComplexValuesPerVector, floatIndex += FloatsPerVector)
        {
            ...
            var values = Vector256.LoadUnsafe(ref sourceReference, (nuint)floatIndex);
            Avx.Multiply(values, factorVector).StoreUnsafe(ref destinationReference, (nuint)floatIndex);
        }
        ...
        Span<ComplexF> destination)
    {
        ...
        for (; index <= leftFloats.Length - FloatsPerVector; index += FloatsPerVector)
        {
            var leftValues = Vector256.LoadUnsafe(ref leftReference, (nuint)index);
            var rightValues = Vector256.LoadUnsafe(ref rightReference, (nuint)index);
            Avx.Add(leftValues, rightValues).StoreUnsafe(ref destinationReference, (nuint)index);
        }
    ...
    private static void EnsureSupported()
    {
        ...
        {
            throw new PlatformNotSupportedException("AVX2/FMA is required by this kernel.");
        }
    }
}
